...
int main(int argc, char* argv[]) {
  ...
  tensorflow::SavedModelBundle bundle1;
  ...
  std::vector<double> latency_record;
  ...
  */
  ...
  std::vector<std::string> output_nodes1;
  auto outputs1 = signature1.outputs();
  printf("Output nodes 1:\n");
  for (auto it = outputs.begin(); it != outputs.end(); it++) {
    output_nodes1.push_back(it->second.name());
    printf("%s\n", it->second.name().c_str());
  }

  std::vector<tensorflow::Tensor> predictions1;

  //int warm = 1;
  for(int i = 0; i < warm; i++) {
    auto status = session1->Run(input_data[rand() % input_size], output_nodes1, {}, &predictions1);
    if (!status.ok()) {
      printf("Failed to run. Error message: %s \n", status.error_message().c_str());
      return 0;
    }
